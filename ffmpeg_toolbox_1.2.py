...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView, QButtonGroup, QStyle, QStackedWidget
)
...
class GenericDownloader(QThread):
    progress = Signal(int)
    status = Signal(str)
    finished_signal = Signal(bool, str)
    
    def __init__(self, download_url=None, name=None):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            temp_dir = Path(__file__).parent / "temp_dl"
            ...
            # Custom Download Mode (e.g. Caesium)
            if self.download_url and self.name:
                downloads.append((self.name, self.download_url))
                if "caesium" in self.name:
                    bin_names = ["caesiumclt.exe"] if system == 'windows' else ["caesiumclt"]
                else:
                    bin_names = [self.name + (".exe" if system == 'windows' else "")]
            # Default FFmpeg Mode
            else:
                if system == 'windows':
            ...
            for name, url in downloads:
                ...
                # Download with progress (simple chunk hook)
                ...
                try:
                    if extract_mode == "zip" or str(dl_path).endswith(".zip"):
                        with zipfile.ZipFile(dl_path, 'r') as zf:
                            zf.extractall(temp_dir)
                    elif extract_mode == "tar" or str(dl_path).endswith(".tar.xz"):
            ...
            if installed_count >= len(bin_names): 
...
class FFmpegRunner(QThread):
    ...
    task_finished_with_cb = Signal(int, str, object) # Signal to run callback on main thread
    ...
    def __init__(self, parent=None):
        ...
        self._is_busy = False
        self.full_log = "" # Accumulate log for parsers (like scene detect)

    def start_task(self, cmd, on_finished=None):
        """Set command and callback, then start thread safely."""
        if self._is_busy or self.isRunning():
            self.log_message.emit("⚠ Runner is busy, please wait...\n")
            return False
        self.cmd = cmd
        self.on_finished_callback = on_finished
        self.full_log = "" # Reset log for new task
        self._is_busy = True
    ...
    def is_busy(self):
        ...
        return self._is_busy or self.isRunning()

    def run(self): # run method of QThread takes no arguments
        args_list = self.cmd # Use the stored command
        on_finished = self.on_finished_callback # Use the stored callback
        
        # Reset
        self.total_duration = None
        
        # Build command
        cmd_str = " ".join(args_list) if isinstance(args_list, list) else args_list
        # Auto-detect duration from input file if possible
        # We need to find the input file path from args to get duration
        # Simple heuristic: find argument after "-i"
        try:
            input_file = None
            parts = shlex.split(cmd_str) # Use shlex.split to handle quoted paths
            for i, part in enumerate(parts):
                if part == "-i" and i < len(parts) - 1:
                    input_file = parts[i+1].strip('"')
                    break
            
            if input_file and os.path.exists(input_file):
                self.total_duration = get_media_duration(input_file)
        except Exception as e:
            self.log_message.emit(f"Warning: Could not detect media duration: {e}\n")

        self.log_message.emit(f"🚀 Executing: {cmd_str}\n")
        if self.total_duration:
            self.log_message.emit(f"ℹ Duration detected: {self.total_duration}s\n")
        ...
        try:
            ...
            # Use list if it's a list, else string with shell=True might be needed, but we prefer list
            run_cmd = args_list
            if isinstance(args_list, str):
                run_cmd = shlex.split(args_list)
            ...
            while self.process.state() != QProcess.NotRunning:
                self.process.waitForReadyRead(100) # Wait for output
                QApplication.processEvents() # Process GUI events
                if self.process.state() == QProcess.NotRunning: # Check again after processing events
                    break
                self.process.waitForFinished(100) # Wait for process to finish
                QApplication.processEvents() # Process GUI events
            ...
            if on_finished:
                # Dispatch callback via signal to ensure it runs on the main thread
                # We pass full_log as the status if it's potentially needed (like for scene detect)
                self.task_finished_with_cb.emit(exit_code, self.full_log or status, on_finished)

        except Exception as e:
            self.log_message.emit(f"❌ Error starting process: {e}\n")
            self.finished.emit(1, str(e))
            if on_finished:
                self.task_finished_with_cb.emit(1, str(e), on_finished)
        finally:
            self._is_busy = False  # Always reset busy flag when done
    ...
    def _stdout(self):
        ...
        if data:
            ...
            self.full_log += data # Capture all output for callbacks
        ...
        if self.process and self.process.state() == QProcess.Running:
            ...
            if not self.process.waitForFinished(1000):
                if os.name == 'nt':
                    ...
                    try:
                        subprocess.run(["taskkill", "/F", "/T", "/PID", str(self.process.processId())], check=True, capture_output=True)
                    except subprocess.CalledProcessError as e:
                        self.log_message.emit(f"Error killing process: {e.stderr.decode()}\n")
                else:
                    self.process.kill()
            self.process = None # Clear the process object
        self._is_busy = False  # Reset busy flag when stopped

    def kill(self):
        self.stop()

    def execute(self, cmd, on_finished=None):
        """Execute a command - wrapper for start_task for backward compatibility."""
        return self.start_task(cmd, on_finished)
...
class YTDLRunner(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_message.emit(f"🚀 Executing: {' '.join(self.cmd)}\n")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            # Splitter: Sidebar | Content -> REPLACED WITH: Top Nav | Content Layout
            # Main Layout: Top Bar (Buttons) -> Center (Splitter: Left Logs | Right Stack) -> Bottom (Progress)
            
            # --- Top Navigation Bar ---
            top_bar = QFrame()
            top_bar.setObjectName("card") # Reuse card style
            ...
            categories = [
                "🎬 Video Basics", "✨ Video Advanced", "🧩 Composition", 
                "🎵 Audio", "📊 Analysis", "🎨 FX / Creative", 
                "🛠 Tools", "⚙ Settings"
            ]
            ...
            # --- Centers Splitter (Left: Logs/Queue | Right: Tools) ---
            center_splitter = QSplitter(Qt.Horizontal)
            layout.addWidget(center_splitter)
            
            # Left Sidebar (Logs, Preview, Queue)
            left_sidebar = QFrame()
            left_sidebar.setFixedWidth(300)
            left_layout = QVBoxLayout(left_sidebar)
            left_layout.setContentsMargins(0, 0, 0, 0)
            center_splitter.addWidget(left_sidebar)
            ...
            self.queue_btn = QPushButton("🚀 Start All")
            ...
            # Right Content Area (Stacked Tabs)
            self.tabs_stack = QStackedWidget()
            center_splitter.addWidget(self.tabs_stack)
            ...
            self.build_caesium_tab() # NEW
            # self.build_subrip_tab() # DELETED (Merged into Subtitles)
            ...
            self.progress = QProgressBar() # Global bottom progress
            ...
            # Runner Init
            self.runner = FFmpegRunner(self)
            self.runner.log_message.connect(self.append_log)
            self.runner.progress.connect(self.progress.setValue) # Connect directly
            self.runner.task_finished_with_cb.connect(self._handle_worker_callback)
    ...
    def build_splitter_tab(self):
        ...
        v = QVBoxLayout(tab)
        v.setSpacing(4)
        self.tabs.addTab(scroll, "✂ Splitter")

        input_card = CardWidget("Input Video")
        self.split_in = QLineEdit()
        self.split_in.setPlaceholderText("Select video file to split...")
        btn = QPushButton("📁 Browse")
        btn.clicked.connect(partial(self.browse_file, self.split_in, "Video (*.mp4 *.mkv *.mov *.avi)"))
        input_card.addRow(self.split_in, btn)
        v.addWidget(input_card)

        mode_card = CardWidget("Split Mode")
        self.split_mode = QComboBox()
        self.split_mode.addItems(["Split by Duration (sec)", "Split into Parts (count)"])
        self.split_mode.currentIndexChanged.connect(self.split_mode_changed)
        
        self.split_val = QDoubleSpinBox()
        self.split_val.setRange(1, 99999)
        self.split_val.setValue(10)
        self.split_lbl = QLabel("Every (sec):")
        
        self.split_method = QComboBox()
        self.split_method.addItems(["Exact (Re-encode)", "Fast (Stream Copy)"])
        self.split_method.setToolTip("Exact: Slower but precise durations.\nFast: Instant but sizes may vary (splits at keyframes).")
        
        mode_card.addRow("Mode:", self.split_mode)
        mode_card.addRow("Method:", self.split_method)
        mode_card.addRow(self.split_lbl, self.split_val)
        v.addWidget(mode_card)

        output_card = CardWidget("Output")
        self.split_outfolder = QLineEdit()
        self.split_outfolder.setPlaceholderText("Output folder...")
        btn_out = QPushButton("📁 Choose")
        btn_out.setObjectName("secondaryBtn")
        btn_out.clicked.connect(partial(self.browse_folder, self.split_outfolder))
        output_card.addRow(self.split_outfolder, btn_out)
        v.addWidget(output_card)

        btn_row = QHBoxLayout()
        self.split_preview_btn = QPushButton("👁 Preview")
        self.split_preview_btn.setObjectName("secondaryBtn")
        self.split_preview_btn.clicked.connect(self.split_preview)
        btn_row.addWidget(self.split_preview_btn)

        self.split_queue_btn = QPushButton("🕒 Queue")
        self.split_queue_btn.clicked.connect(partial(self.generic_add_queue, self.split_preview, "Splitter", self.split_in))
        btn_row.addWidget(self.split_queue_btn)

        self.split_run_btn = QPushButton("▶ Split Video")
        self.split_run_btn.clicked.connect(self.split_run)
        btn_row.addWidget(self.split_run_btn)
        v.addLayout(btn_row)
        v.addStretch()
        scroll.setWidget(tab)
    ...
    def build_img2vid_tab(self):
        tab = QWidget()
        v = QVBoxLayout(tab)
        v.setSpacing(4)
        self.tabs.addTab(tab, "🖼 Img → Vid")

        list_card = CardWidget("Images")
        self.i2v_list = QListWidget()
        self.i2v_list.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.i2v_list.setMinimumHeight(120)
        list_card.content_layout.addWidget(self.i2v_list)
        
        btn_h = QHBoxLayout()
        btn_add = QPushButton("➕ Add Images")
        btn_add.clicked.connect(self.i2v_add_files)
        btn_h.addWidget(btn_add)
        btn_rem = QPushButton("➖ Remove")
        btn_rem.setObjectName("secondaryBtn")
        btn_rem.clicked.connect(self.i2v_remove)
        btn_h.addWidget(btn_rem)
        btn_clr = QPushButton("🗑 Clear")
        btn_clr.setObjectName("secondaryBtn")
        btn_clr.clicked.connect(self.i2v_list.clear)
        btn_h.addWidget(btn_clr)
        list_card.content_layout.addLayout(btn_h)
        v.addWidget(list_card)

        sets_card = CardWidget("Settings")
        self.i2v_dur = QDoubleSpinBox()
        self.i2v_dur.setRange(0.1, 3600.0)
        self.i2v_dur.setValue(10.0)
        self.i2v_dur.setSuffix(" sec")
        
        self.i2v_fps = QSpinBox()
        self.i2v_fps.setRange(1, 120)
        self.i2v_fps.setValue(30)
        self.i2v_fps.setSuffix(" fps")
        
        sets_card.addRow("Duration per Video:", self.i2v_dur, "Frame Rate:", self.i2v_fps)
        v.addWidget(sets_card)

        output_card = CardWidget("Output")
        self.i2v_outfolder = QLineEdit()
        self.i2v_outfolder.setPlaceholderText("Output folder...")
        btn_out = QPushButton("📁 Choose")
        btn_out.setObjectName("secondaryBtn")
        btn_out.clicked.connect(partial(self.browse_folder, self.i2v_outfolder))
        output_card.addRow(self.i2v_outfolder, btn_out)
        v.addWidget(output_card)

        btn_row = QHBoxLayout()
        self.i2v_preview_btn = QPushButton("👁 Preview")
        self.i2v_preview_btn.setObjectName("secondaryBtn")
        self.i2v_preview_btn.clicked.connect(self.i2v_preview)
        btn_row.addWidget(self.i2v_preview_btn)

        self.i2v_queue_btn = QPushButton("🕒 Queue")
        self.i2v_queue_btn.clicked.connect(partial(self.generic_add_queue, self.i2v_preview, "Img2Vid", None))
        btn_row.addWidget(self.i2v_queue_btn)

        self.i2v_run_btn = QPushButton("▶ Create Videos")
        self.i2v_run_btn.clicked.connect(self.i2v_run)
        btn_row.addWidget(self.i2v_run_btn)
        v.addLayout(btn_row)

    def i2v_add_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Images", self.last_dir, "Images (*.jpg *.png *.jpeg *.bmp *.webp)")
        if files:
            self.last_dir = str(Path(files[0]).parent)
            self.i2v_list.addItems(files)

    def i2v_remove(self):
        for item in self.i2v_list.selectedItems():
            self.i2v_list.takeItem(self.i2v_list.row(item))
    ...
    def build_convert_tab(self):
        ...
        btn_save_pre = QPushButton("💾 Save")
        btn_save_pre.clicked.connect(self.save_preset_ui)
        btn_del_pre = QPushButton("🗑 Delete")
        ...
        self.conv_queue_btn = QPushButton("🕒 Queue")
    ...
    def append_log(self, text):
        ...
        try:
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                # Add timestamp if it's a new line or not empty
                if text.strip():
    ...
    def build_caesium_tab(self):
        tab = QWidget()
        v = QVBoxLayout(tab)
        v.setSpacing(4)
        self.tabs.addTab(tab, "🗜 Image Compress")

        list_card = CardWidget("Images")
        self.ae_list = QListWidget()
        self.ae_list.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.ae_list.setMinimumHeight(100)
        list_card.content_layout.addWidget(self.ae_list)
        
        btn_h = QHBoxLayout()
        btn_add = QPushButton("➕ Add Images")
        btn_add.clicked.connect(self.caesium_add_files)
        btn_h.addWidget(btn_add)
        btn_rem = QPushButton("➖ Remove")
        btn_rem.setObjectName("secondaryBtn")
        btn_rem.clicked.connect(self.caesium_remove)
        btn_h.addWidget(btn_rem)
        btn_clr = QPushButton("🗑 Clear")
        btn_clr.setObjectName("secondaryBtn")
        btn_clr.clicked.connect(self.ae_list.clear)
        btn_h.addWidget(btn_clr)
        list_card.content_layout.addLayout(btn_h)
        v.addWidget(list_card)

        sets_card = CardWidget("Compression Ratio")
        self.ae_qual = QSlider(Qt.Horizontal)
        self.ae_qual.setRange(0, 100)
        self.ae_qual.setValue(80)
        self.ae_qual_lbl = QLabel("Quality: 80")
        self.ae_qual.valueChanged.connect(lambda val: self.ae_qual_lbl.setText(f"Quality: {val}"))
        
        sets_card.addRow(self.ae_qual_lbl, self.ae_qual)
        self.ae_lossless = QCheckBox("Lossless")
        self.ae_keep_meta = QCheckBox("Keep Metadata")
        sets_card.content_layout.addWidget(self.ae_lossless)
        sets_card.content_layout.addWidget(self.ae_keep_meta)
        v.addWidget(sets_card)

        output_card = CardWidget("Output")
        self.ae_outfolder = QLineEdit()
        self.ae_outfolder.setPlaceholderText("Output folder (Defaults to source parent + /compressed)...")
        btn_out = QPushButton("📁 Choose")
        btn_out.setObjectName("secondaryBtn")
        btn_out.clicked.connect(partial(self.browse_folder, self.ae_outfolder))
        output_card.addRow(self.ae_outfolder, btn_out)
        v.addWidget(output_card)

        btn_row = QHBoxLayout()
        self.ae_preview_btn = QPushButton("👁 Preview")
        self.ae_preview_btn.setObjectName("secondaryBtn")
        self.ae_preview_btn.clicked.connect(self.caesium_preview)
        btn_row.addWidget(self.ae_preview_btn)

        self.ae_run_btn = QPushButton("▶ Compress Images")
        self.ae_run_btn.clicked.connect(self.caesium_run)
        btn_row.addWidget(self.ae_run_btn)
        v.addLayout(btn_row)

    def caesium_add_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Images", self.last_dir, "Images (*.jpg *.jpeg *.png *.webp *.tiff)")
        if files:
            self.last_dir = str(Path(files[0]).parent)
            self.ae_list.addItems(files)

    def caesium_remove(self):
        for item in self.ae_list.selectedItems():
            self.ae_list.takeItem(self.ae_list.row(item))

    def caesium_preview(self):
        count = self.ae_list.count()
        if count == 0: 
            self.preview.setPlainText("No images added.")
            return

        bin_path = get_binary("caesiumclt")
        
        # Example command
        args = [bin_path]
        if self.ae_lossless.isChecked(): args.append("--lossless")
        else: args.extend(["--quality", str(self.ae_qual.value())])
        
        if self.ae_keep_meta.isChecked(): args.append("--keep-dates"); args.append("--exif") # approximate flags
        
        args.extend(["--output", "OUTPUT_FOLDER", "INPUT_FILES..."])
        
        self.preview.setPlainText(" ".join(args) + f"\n\n(Will process {count} files)")

    def caesium_run(self):
        count = self.ae_list.count()
        if count == 0: return
        
        bin_path = get_binary("caesiumclt")
        if not os.path.exists(bin_path) and not shutil.which("caesiumclt"):
            QMessageBox.critical(self, "Missing Dependency", "CaesiumCLT not found. Please download it from Settings.")
            return
            
        qual = self.ae_qual.value()
        is_lossless = self.ae_lossless.isChecked()
        keep_meta = self.ae_keep_meta.isChecked()
        
        # Group by output folder to optimize batching if possible, but simplest is loop
        # CaesiumCLI supports multiple files, but output must be a directory
        
        files = [self.ae_list.item(i).text() for i in range(count)]
        
        # We can run one big command if they all go to same output
        # But if we want specific output folder:
        out_root = self.ae_outfolder.text().strip()
        
        # If output folder is specified, we can batch all files.
        # If not, we might want "same as input" or "subfolder". 
        # CaesiumCLI -o takes a directory. 
        if not out_root:
             # Default to source_parent/compressed
             # Logic: Group files by parent dir? Or just process one by one.
             # One by one is safer for mixed sources.
             pass
        ...
        for f in files:
            out_dir = out_root
            if not out_dir:
                out_dir = str(Path(f).parent / "compressed")
            ...
            if is_lossless: 
                ...
                args.extend(["-q", str(qual)])
            
            if keep_meta: 
                args.append("--keep-dates")
        ...
        def run_next(idx=0):
            if idx >= len(cmds):
                self._on_finished("Image Compression", 0, 0)
                return
            self.runner.execute(cmds[idx], on_finished=lambda c, s, i=idx: run_next(i + 1))
            
        run_next()

    def i2v_preview(self):
        count = self.i2v_list.count()
        if count == 0:
            self.preview.setPlainText("No images added.")
            return

        dur = self.i2v_dur.value()
        fps = self.i2v_fps.value()
        
        # Just show first one as example
        img = self.i2v_list.item(0).text()
        outfolder = self.i2v_outfolder.text().strip() or str(Path(img).parent)
        outp = str(Path(outfolder) / (Path(img).stem + ".mp4"))
        
        # Command: ffmpeg -loop 1 -i img -c:v libx264 -t dur -pix_fmt yuv420p -r fps out.mp4
        cmd = [get_binary("ffmpeg"), "-loop", "1", "-i", img, 
               "-c:v", "libx264", "-t", str(dur), "-pix_fmt", "yuv420p", "-r", str(fps), "-y", outp]
        
        text = f"# Batch of {count} images. Example command:\n" + " ".join(map(quote, cmd))
    ...
    def i2v_run(self):
        ...
        def run_next(idx=0):
            if idx >= len(cmds):
                self._on_finished("Img2Vid Batch", 0, 0)
                return
            
            cmd_list = cmds[idx]
            self.runner.execute(cmd_list, on_finished=lambda ec, st, i=idx: run_next(i + 1))
            
        run_next()

    def split_preview(self):
        inp = self.split_in.text().strip()
        if not inp: return
        
        mode = self.split_mode.currentIndex() # 0=Dur, 1=Parts
        val = self.split_val.value()
        outfolder = self.split_outfolder.text().strip() or str(Path(inp).parent)
        
        # We need output pattern: input_001.mp4
        name_stem = Path(inp).stem
        ext = Path(inp).suffix
        segment_pattern = str(Path(outfolder) / f"{name_stem}_%03d{ext}")
        
        cmd = [get_binary("ffmpeg"), "-i", inp, "-c", "copy", "-map", "0", "-f", "segment"]
        
        if mode == 0: # Duration
            cmd.extend(["-segment_time", str(val)])
        else: # Parts
            # Must get total duration first
            total = get_media_duration(inp)
            if not total:
                self.preview.setPlainText("Could not detect duration of input file for parts calculation.")
                return
            seg_time = total / val
            cmd.extend(["-segment_time", str(seg_time)])
            
        cmd.extend(["-reset_timestamps", "1", "-y", segment_pattern])
        self.preview.setPlainText(" ".join(map(quote, cmd)))

    def split_run(self):
        self.split_preview()
        cmd_str = self.preview.toPlainText()
        if "Could not detect" in cmd_str or not cmd_str: return
        
        import shlex
        args = shlex.split(cmd_str)
        self.runner.execute(args, on_finished=lambda c, s: self._on_finished("Splitter", c, s))
    ...
    def build_extract_tab(self):
        ...
        self.ext_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_merge_tab(self):
        ...
        self.mg_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_trim_tab(self):
        ...
        self.trim_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_watermark_tab(self):
        ...
        self.wm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_subtitles_tab(self):
        self.tabs.addTab(self.build_subtitles_container(), "📝 Subtitles")

    def build_subtitles_container(self):
        # Container widget with internal tabs
        container = QWidget()
        ...
        self.sub_file.setPlaceholderText("Select subtitle file (srt/ass)...")
        ...
        self.sub_queue_btn = QPushButton("🕒 Queue")
        ...
        sub_tabs.addTab(t1, "Burn/Embed")
        
        # Tab 2: Extract/Rip (Logic from build_subrip_tab)
        t2 = QWidget()
        v2 = QVBoxLayout(t2)
        
        ri_card = CardWidget("Input Video")
        self.subrip_in = QLineEdit()
        self.subrip_in.setPlaceholderText("Select video file...")
        btn_ri = QPushButton("📁 Browse")
        btn_ri.clicked.connect(partial(self.browse_file, self.subrip_in, "Video (*.mkv *.mp4 *.mov);;All (*)"))
        ri_card.addRow(self.subrip_in, btn_ri)
        
        btn_scan = QPushButton("🔍 Scan Subtitles")
        btn_scan.clicked.connect(self.subrip_scan)
        ri_card.content_layout.addWidget(btn_scan)
        v2.addWidget(ri_card)
        ...
        self.subrip_info.setMaximumHeight(100)
        info_card.content_layout.addWidget(self.subrip_info)
        v2.addWidget(info_card)

        rs_card = CardWidget("Settings")
        self.subrip_idx = QSpinBox(); self.subrip_idx.setRange(0, 99)
        self.subrip_fmt = QComboBox(); self.subrip_fmt.addItems(["srt", "ass", "vtt"])
        rs_card.addRow("Track Index (s:):", self.subrip_idx, "Format:", self.subrip_fmt)
        v2.addWidget(rs_card)

        ro_card = CardWidget("Output")
        self.subrip_outfolder = QLineEdit()
        self.subrip_outfolder.setPlaceholderText("Output folder...")
        btn_ro = QPushButton("📁 Choose")
        btn_ro.setObjectName("secondaryBtn")
        btn_ro.clicked.connect(partial(self.browse_folder, self.subrip_outfolder))
        ro_card.addRow(self.subrip_outfolder, btn_ro)
        self.subrip_custom = QLineEdit() # Added missing custom output for subrip
        self.subrip_custom.setPlaceholderText("Custom output name (optional)...")
        ro_card.addRow("Name:", self.subrip_custom)
        v2.addWidget(ro_card)
    ...
    def build_merge_multi_tab(self):
        ...
        self.mm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_slideshow_tab(self):
        ...
        self.ss_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_gif_tab(self):
        ...
        self.gif_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_resize_tab(self):
        ...
        self.resize_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_batch_tab(self):
        ...
        self.batch_queue_btn = QPushButton("🕒 Queue")
        ...
        self.batch_stop_btn.setObjectName("secondaryBtn")
    ...
    def build_compress_tab(self):
        ...
        self.comp_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_speed_tab(self):
        ...
        self.speed_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_metadata_tab(self):
        ...
        self.meta_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_reverse_tab(self):
        ...
        self.rev_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_normalize_tab(self):
        ...
        self.norm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_frames_tab(self):
        ...
        self.frm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_stab_tab(self):
        ...
        self.stab_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_delogo_tab(self):
        ...
        self.dl_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_color_tab(self):
        ...
        self.col_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_waveform_tab(self):
        ...
        self.wav_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_stream_tab(self):
        ...
        self.str_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_smartcut_tab(self):
        ...
        self.sc_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_scene_tab(self):
        ...
        self.scene_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_webopt_tab(self):
        ...
        self.web_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_pip_tab(self):
        ...
        self.pip_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_cleaner_tab(self):
        ...
        self.clean_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_grid_tab(self):
        ...
        self.grid_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_yt_tab(self):
        ...
        self.yt_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_lut_tab(self):
        ...
        self.lut_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_scopes_tab(self):
        ...
        self.scp_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_mosaic_tab(self):
        ...
        self.mos_info = QCheckBox("Show File Info at Bottom")
        self.mos_info.setChecked(True)
        
        sets_card.content_layout.addWidget(self.mos_labels)
        sets_card.content_layout.addWidget(self.mos_info)
        v.addWidget(sets_card)
        ...
        self.mos_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_visualizer_tab(self):
        ...
        self.vis_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_tonemap_tab(self):
        ...
        self.tm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_slowmo_tab(self):
        ...
        self.sm_queue_btn = QPushButton("🕒 Queue")
    ...
    def build_caesium_tab(self):
        ...
        v = QVBoxLayout(tab)
        v.setSpacing(4)
        self.tabs.addTab(scroll, "🌄 Image Compress")

        input_card = CardWidget("Images to Compress")
        self.caesium_list = QListWidget()
        self.caesium_list.setMinimumHeight(80)
        input_card.content_layout.addWidget(self.caesium_list)
        ...
        btn_rem.clicked.connect(self.caesium_remove)
        btn_h.addWidget(btn_rem)
        input_card.content_layout.addLayout(btn_h)
        v.addWidget(input_card)

        sets_card = CardWidget("Compression Settings")
        self.caesium_quality = QSpinBox()
        self.caesium_quality.setRange(1, 100)
        self.caesium_quality.setValue(80)
        self.caesium_quality.setSuffix("%")
        
        self.caesium_format = QComboBox()
        self.caesium_format.addItems(["jpg", "png", "webp"])
        
        sets_card.addRow("Quality:", self.caesium_quality, "Format:", self.caesium_format)
        v.addWidget(sets_card)

        output_card = CardWidget("Output Directory")
        self.caesium_outfolder = QLineEdit()
        self.caesium_outfolder.setPlaceholderText("Output folder...")
        btn_out = QPushButton("📁 Choose")
        btn_out.setObjectName("secondaryBtn")
        btn_out.clicked.connect(partial(self.browse_folder, self.caesium_outfolder))
        output_card.addRow(self.caesium_outfolder, btn_out)
        v.addWidget(output_card)

        btn_row = QHBoxLayout()
        self.caesium_preview_btn = QPushButton("👁 Preview")
        self.caesium_preview_btn.setObjectName("secondaryBtn")
        self.caesium_preview_btn.clicked.connect(self.caesium_preview)
        btn_row.addWidget(self.caesium_preview_btn)

        self.caesium_queue_btn = QPushButton("🕒 Queue")
        self.caesium_queue_btn.clicked.connect(partial(self.generic_add_queue, self.caesium_preview, "Caesium", None))
        btn_row.addWidget(self.caesium_queue_btn)

        self.caesium_run_btn = QPushButton("▶ Compress Images")
        self.caesium_run_btn.clicked.connect(self.caesium_run)
        btn_row.addWidget(self.caesium_run_btn)
        v.addLayout(btn_row)
        v.addStretch()
        scroll.setWidget(tab)
    ...
    def build_settings_tab(self):
        ...
        # Dependencies Status
        dep_card = CardWidget("Dependencies Status")
        self.dep_status_lbl = QLabel()
        self.check_dependencies_status()
        dep_card.content_layout.addWidget(self.dep_status_lbl)
        v.addWidget(dep_card)
        
        action_card = CardWidget("Updates & Downloads")
        
        # FFmpeg
        h_ff = QHBoxLayout()
        h_ff.addWidget(QLabel("FFmpeg:"))
        btn_ff = QPushButton("⬇ Download/Update")
        btn_ff.clicked.connect(self.start_download_ffmpeg)
        h_ff.addWidget(btn_ff)
        action_card.content_layout.addLayout(h_ff)

        # Caesium
        h_ae = QHBoxLayout()
        h_ae.addWidget(QLabel("Caesium:"))
        btn_ae = QPushButton("⬇ Download/Update")
        btn_ae.clicked.connect(self.start_download_caesium)
        h_ae.addWidget(btn_ae)
        action_card.content_layout.addLayout(h_ae)
        ...
        log_card = CardWidget("Application Log")
        self.log_output = QTextEdit()
        self.log_output.setReadOnly(True)
        self.log_output.setMaximumHeight(200)
        log_card.content_layout.addWidget(self.log_output)
        ...
        log_card.content_layout.addLayout(log_controls_layout)
    ...
    def check_dependencies_status(self):
        ...
        # Check FFmpeg
        bin_ff = "ffmpeg.exe" if os.name == 'nt' else "ffmpeg"
        if (BINS_DIR / bin_ff).exists():
            status_text += f"<span style='color: #4cd137'><b>FFmpeg:</b> Found (Local)</span><br>"
        elif shutil.which("ffmpeg"):
            status_text += f"<span style='color: #4cd137'><b>FFmpeg:</b> Found (System)</span><br>"
        else:
            status_text += f"<span style='color: #e84118'><b>FFmpeg:</b> NOT Found</span><br>"
            
        # Check Caesium
        bin_ae = "caesiumclt.exe" if os.name == 'nt' else "caesiumclt"
        if (BINS_DIR / bin_ae).exists():
            status_text += f"<span style='color: #4cd137'><b>Caesium:</b> Found (Local)</span>"
        else:
            status_text += f"<span style='color: #e84118'><b>Caesium:</b> NOT Found (Required for Image Compress tab)</span>"
            
        self.dep_status_lbl.setText(status_text)
        
    def start_download_caesium(self):
        url = "https://github.com/Saerasoft/caesium-clt/releases/download/1.0.0-beta4/caesiumclt-windows-x86_64.zip" 
        # Note: Hardcoded specific version for reliability, or we could scrape latest.
        # But Beta4 is stable enough for now.
        self.start_generic_download(url, "caesiumclt")

    def start_generic_download(self, url, name):
        self.dl_progress.setValue(0)
        self.dl_progress.setVisible(True)
        self.downloader = GenericDownloader(url, name)
    ...
    def start_download_ffmpeg(self):
        self.dl_progress.setValue(0)
        self.dl_progress.setVisible(True)
        # Empty args trigger default FFmpeg logic
        ...
        if vcodec != "copy":
            ...
            if "CRF" in mode:
                ...
                if self.conv_two_pass.isChecked():
                     pass_log = TEMP_DIR / "ffmpeg2pass"
                     # Simplify for preview: just show 2nd pass logic note or first pass
                     pass_args = ["-passlogfile", quote(pass_log)]
                     cmd = ["# 2-Pass Encoding (Pass 1)\n"] + cmd + ["-pass", "1"] + pass_args + ["-f", "null", "NUL" if os.name=='nt' else "/dev/null", "&&", "\n# Pass 2\n"] + [get_binary("ffmpeg"), "-i", inp, "-c:v", vcodec, "-b:v", f"{vb}k", "-pass", "2"] + pass_args
        else:
            cmd += ["-c:v", "copy"]
    ...
    def conv_run(self):
        ...
        # Split by newline or && for multi-pass
        cmds = [c.strip() for c in re.split(r'\n|&&', text) if c.strip() and not c.strip().startswith("#")]
        
        def run_chain(idx=0):
            if idx >= len(cmds):
                # cleanup logs
                pass_log = str(TEMP_DIR / "ffmpeg2pass")
                for ext in ["-0.log", "-0.log.mbtree"]:
                    p = Path(pass_log + ext)
                    if p.exists():
                        try: p.unlink()
                        except: pass
                self._on_finished("Convert", 0, "Done")
                return
            
            self.runner.execute(cmds[idx], on_finished=lambda ec, st: run_chain(idx+1) if ec==0 else self._on_finished("Convert Failed", ec, st))
        
        if len(cmds) > 1:
            run_chain()
        else:
            self.runner.execute(cmds[0], on_finished=lambda ec, st: self._on_finished("Convert", ec, st))
    ...
    def ext_run(self):
        ...
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Extract", ec, st))
    ...
    def mg_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Merge", ec, st))
    ...
    def trim_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Trim", ec, st))
    ...
    def wm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Watermark", ec, st))
    ...
    def sub_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Subtitles", ec, st))
    ...
    def mm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Merge Multi", ec, st))
    ...
    def ss_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Slideshow", ec, st))

    # ==================== IMAGE TO VIDEO ====================
    def i2v_preview(self):
        count = self.i2v_list.count()
        if count == 0:
            QMessageBox.warning(self, "No Images", "Add images to convert.")
            return
        
        dur = self.i2v_dur.value()
        fps = self.i2v_fps.value()
        outfolder = self.i2v_outfolder.text().strip()
        ...
        # Show command for first image as preview
        first_img = self.i2v_list.item(0).text()
        outp = str(Path(outfolder) / (Path(first_img).stem + "_video.mp4"))
        
        cmd = [get_binary("ffmpeg"), "-loop", "1", "-i", first_img, "-t", str(dur), 
               "-r", str(fps), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-y", outp]
        self.preview.setPlainText(f"Processing {count} images. Example command:\n" + " ".join(map(quote, cmd)))
    ...
    def i2v_run(self):
        ...
        def run_next(idx=0):
            if idx >= count:
                self._on_finished("Image to Video", 0, f"Processed {count} images")
                return
            
            img_path = self.i2v_list.item(idx).text()
            outp = str(Path(outfolder) / (Path(img_path).stem + "_video.mp4"))
            cmd = [get_binary("ffmpeg"), "-loop", "1", "-i", img_path, "-t", str(dur),
                   "-r", str(fps), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-y", outp]
            self.runner.execute(cmd, on_finished=lambda ec, st: run_next(idx+1) if ec==0 else self._on_finished(f"Image2Vid Failed at {idx+1}", ec, st))
        
        run_next()

    # ==================== VIDEO SPLITTER ====================
    def split_mode_changed(self, idx):
        if idx == 0:
            self.split_lbl.setText("Every (sec):")
            self.split_val.setSuffix(" sec")
            self.split_val.setDecimals(1)
        else:
            self.split_lbl.setText("Number of parts:")
            self.split_val.setSuffix(" parts")
            self.split_val.setDecimals(0)

    def split_preview(self):
        inp = self.split_in.text().strip()
        if not inp:
            self.preview.setPlainText("Select video file to split.")
            return
        ...
        outp = str(Path(outfolder) / "chunk_%03d.mp4")
        
        method = self.split_method.currentIndex()
        if method == 0: # Exact (Re-encode)
            codec = ["-c:v", "libx264", "-crf", "23", "-c:a", "aac", "-b:a", "128k"]
        else: # Fast (Stream Copy)
            codec = ["-c", "copy"]

        if mode == 0:  # Split by duration
            cmd = [get_binary("ffmpeg"), "-i", inp, "-f", "segment", "-segment_time", str(val),
                   "-reset_timestamps", "1"] + codec + ["-y", outp]
        else:  # Split into parts
            total_dur = get_media_duration(inp)
            if total_dur:
                part_dur = total_dur / val
                cmd = [get_binary("ffmpeg"), "-i", inp, "-f", "segment", "-segment_time", str(round(part_dur, 3)),
                       "-reset_timestamps", "1"] + codec + ["-y", outp]
            else:
                cmd = [get_binary("ffmpeg"), "-i", inp, "-f", "segment", "-segment_time", "60",
                       "-reset_timestamps", "1"] + codec + ["-y", outp]
    ...
    def split_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Video Split", ec, st))
    ...
    def batch_run(self):
        ...
        def run_next(idx=0):
            if self._batch_cancelled:
                ...
                self._on_finished("Batch", -1, "Cancelled")
                on_batch_finished()
                return
            if idx >= len(lines):
            ...
            if not cmdline or cmdline.startswith("#"):
                self.append_log(f"⏭ Skipping: {cmdline}\n")
                run_next(idx + 1)
                return
            self.runner.execute(cmdline, on_finished=lambda ec, st, i=idx: run_next(i + 1) if not self._batch_cancelled else on_batch_finished())
        run_next()
    ...
    def gif_preview(self):
        ...
        if hq:
            # Use temp folder for palette
            pal_path = TEMP_DIR / "palette.png"
            pal_cmd = f"{quote(get_binary('ffmpeg'))} -ss {start} -t {dur} -i {quote(inp)} -vf \"{vf},palettegen\" -y {quote(pal_path)}"
            gif_cmd = f"{quote(get_binary('ffmpeg'))} -ss {start} -t {dur} -i {quote(inp)} -i {quote(pal_path)} -filter_complex \"{vf} [x]; [x][1:v] paletteuse\" -y {quote(outp)}"
            if os.name == 'nt':
                cmd = f"{pal_cmd}\n{gif_cmd}"
            else:
    ...
    def gif_run(self):
        ...
        def run_chain(idx=0):
            ...
            self.runner.execute(cmd, on_finished=lambda ec, st: run_chain(idx+1) if ec==0 else self._on_finished("GIF Failed", ec, st))
    ...
    def resize_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Resize", ec, st))
    ...
    def comp_preview(self):
        ...
        pass_log = TEMP_DIR / "ffmpeg2pass"
        pass1 = f"{quote(get_binary('ffmpeg'))} -y -i {quote(inp)} -c:v libx264 -b:v {video_bitrate_k}k -passlogfile {quote(pass_log)} -pass 1 -an -f null {null_out}"
        pass2 = f"{quote(get_binary('ffmpeg'))} -y -i {quote(inp)} -c:v libx264 -b:v {video_bitrate_k}k -passlogfile {quote(pass_log)} -pass 2 -c:a aac -b:a {audio_k}k {quote(outp)}"
    ...
    def comp_run(self):
        ...
        def run_chain(idx=0):
            if idx >= len(cmds):
                ...
                return
            cmd = cmds[idx].strip()
            self.runner.execute(cmd, on_finished=lambda ec, st: run_chain(idx+1) if ec==0 else self._on_finished("Compression Failed", ec, st))
    ...
    def speed_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Speed Change", ec, st))
    ...
    def meta_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Metadata Update", ec, st))
    ...
    def rec_run(self):
        ...
        self.preview.setPlainText(" ".join(map(quote, cmd)))
        self.runner.execute(cmd, on_finished=self._on_rec_finished)
        
        self.rec_start_btn.setEnabled(False)
        self.rec_start_btn.setText("🔴 Recording...")
        self.rec_stop_btn.setEnabled(True)
        self.rec_status_lbl.setText("🔴 Recording active")
        self.rec_status_lbl.setStyleSheet("color: #e84118; font-weight: bold;")

    def _on_rec_finished(self, ec, status):
        self.rec_start_btn.setEnabled(True)
        self.rec_start_btn.setText("🔴 Start Recording")
        self.rec_stop_btn.setEnabled(False)
        self.rec_status_lbl.setText("Ready to record")
        self.rec_status_lbl.setStyleSheet("")
        self._on_finished("Recording", ec, status)

    def rec_stop(self):
        self.runner.stop()
        # UI reset handles in _on_rec_finished
        self.rec_status_lbl.setText("Stopping...")
    ...
    def rev_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Reverse", ec, st))
    ...
    def norm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Normalization", ec, st))
    ...
    def frm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Frame Extraction", ec, st))
    ...
    def stab_preview(self):
        ...
        trf = TEMP_DIR / "transforms.trf"
        p1 = f"{quote(get_binary('ffmpeg'))} -y -i {quote(inp)} -vf vidstabdetect=shakiness=10:accuracy=15:result={quote(trf)} -f null NUL"
        p2 = f"{quote(get_binary('ffmpeg'))} -y -i {quote(inp)} -vf vidstabtransform=smoothing={sm}:input={quote(trf)} -c:v libx264 -crf 18 -pix_fmt yuv420p {quote(outp)}"
    ...
    def stab_run(self):
        ...
        def run_chain(idx=0):
            ...
            cmd = cmds[idx].strip()
            self.runner.execute(cmd, on_finished=lambda ec, st: run_chain(idx+1) if ec==0 else self._on_finished("Stab Failed", ec, st))
        run_chain()
    ...
    def dl_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Delogo", ec, st))
    ...
    def col_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Color Adjustment", ec, st))
    ...
    def wav_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Waveform Generation", ec, st))
    ...
    def str_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Stream Remux", ec, st))
    ...
    def sc_run(self):
        ...
        cmd = [get_binary("ffmpeg"), "-i", inp, "-af", f"silencedetect=noise={t}dB:d={d}", "-f", "null", "-"]
        self.runner.execute(cmd, on_finished=on_analyzed)
...
"""
    ...
    def scene_run(self):
        ...
        def on_scanned(ec, status):
            ...
            self.runner.execute(cmd, on_finished=lambda ec2, st2: self._on_finished("Scene Split", ec2, st2))

        # Pass 1: Scan for scenes
        cmd = [get_binary("ffmpeg"), "-i", inp, "-vf", f"select='gt(scene,{v})',metadata=mode=print", "-f", "null", "-"]
        self.runner.execute(cmd, on_finished=on_scanned)
    ...
    def subrip_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Subtitle Rip", ec, st))
    ...
    def web_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Web Optimization", ec, st))
    ...
    def pip_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("PIP Generation", ec, st))
    ...
    def clean_run(self):
        ...
        def run_next(idx=0):
            ...
            self.runner.execute(cmd, on_finished=lambda ec, st: run_next(idx+1) if ec==0 else self._on_finished("Batch Clean Failed", ec, st))
    ...
    def soc_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Social Crop", ec, st))
    ...
    def grid_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Video Grid", ec, st))
    ...
    def yt_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("YT Video Create", ec, st))
    ...
    def lut_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("LUT App", ec, st))
    ...
    def scp_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Scopes Gen", ec, st))
    ...
    def prx_preview(self, is_queue=False):
        ...
        if self.prx_burn_tc.isChecked():
             ...
             vf.append(f"drawtext=fontfile='{font_path}':text='TC\\:%{{pts\\:hms}}':x=10:y=H-30:fontcolor=white:fontsize=24:box=1:boxcolor=black@0.5")
        if self.prx_burn_name.isChecked():
             name_esc = Path(inp).name.replace(":", "\\:")
             vf.append(f"drawtext=fontfile='{font_path}':text='{name_esc}':x=10:y=10:fontcolor=white:fontsize=18:box=1:boxcolor=black@0.5")
    ...
    def prx_run(self):
        ...
        if cmd:
             self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Proxy Gen", ec, st))
    ...
    def mos_preview(self):
        ...
        if self.mos_labels.isChecked():
            vf.append(f"drawtext=fontfile='{font_path}':text='%{{pts\\:hms}}':x=10:y=h-20:fontcolor=white:fontsize=16:box=1:boxcolor=black@0.5")
            
        vf.append(f"scale={width//cols}:-1")
        vf.append(f"tile={cols}x{rows}")
        
        if hasattr(self, 'mos_info') and self.mos_info.isChecked():
            # Get info for footer
            fname = Path(inp).name
            fsize = os.path.getsize(inp) / (1024*1024)
            info_text = f"File: {fname} | Size: {fsize:.1f} MB | Duration: {dur:.1f}s"
            # Pad bottom by 40px and draw info
            vf.append(f"pad=iw:ih+40:0:0:black")
            vf.append(f"drawtext=fontfile='{font_path}':text='{info_text}':x=10:y=h-30:fontcolor=white:fontsize=20")
    ...
    def mos_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Mosaic Gen", ec, st))
    ...
    def vis_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Visualizer Gen", ec, st))
    ...
    def tm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Tone Mapping", ec, st))
    ...
    def sm_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Flow Slowmo", ec, st))

    # ==================== CAESIUM IMAGE COMPRESSOR LOGIC ====================
    def caesium_add_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Images", self.last_dir, "Images (*.jpg *.png *.jpeg *.webp *.bmp);;All (*)")
        if files:
            self.last_dir = str(Path(files[0]).parent)
            save_config({"last_dir": self.last_dir})
            for f in files:
                self.caesium_list.addItem(f)

    def caesium_remove(self):
        for it in self.caesium_list.selectedItems():
            self.caesium_list.takeItem(self.caesium_list.row(it))

    def caesium_preview(self):
        count = self.caesium_list.count()
        if count == 0:
            QMessageBox.warning(self, "No Images", "Add images to compress.")
            return
        
        outfolder = self.caesium_outfolder.text().strip()
        if not outfolder:
            QMessageBox.warning(self, "Output Missing", "Select output folder.")
            return
        
        quality = self.caesium_quality.value()
        fmt = self.caesium_format.currentText()
        first_img = self.caesium_list.item(0).text()
        
        cmd = [get_binary("caesiumclt"), "-q", str(quality), "-f", fmt, "-o", outfolder, first_img]
        self.preview.setPlainText(f"Processing {count} images. Example:\n" + " ".join(map(quote, cmd)))
    ...
    def caesium_run(self):
        ...
        # Get all images
        images = [self.caesium_list.item(i).text() for i in range(count)]
        
        # Caesium can take multiple files
        cmd = [get_binary("caesiumclt"), "-q", str(quality), "-f", fmt, "-o", outfolder] + images
        self.preview.setPlainText(" ".join(map(quote, cmd)))
        
        # Run via subprocess (not FFmpegRunner since it's not FFmpeg)
        try:
            startupinfo = None
            if os.name == 'nt':
                startupinfo = subprocess.STARTUPINFO()
            ...
            self.append_log(f"🚀 Running Caesium: {' '.join(cmd)}\n")
            ...
            if result.returncode == 0:
                QMessageBox.information(self, "Success", f"Compressed {count} images!")
            else:
                QMessageBox.warning(self, "Error", f"Caesium exited with code {result.returncode}")
        except FileNotFoundError:
            QMessageBox.critical(self, "Caesium Not Found", "caesiumclt not found. Please download it from Settings tab.")
        except Exception as e:
    ...
    def process_next_queue_item(self):
        ...
        self.runner.execute(item["cmd"], on_finished=lambda ec, st, i=idx: self.on_queue_item_finished(i, ec, st))
    ...
    def _handle_worker_callback(self, code, status, callback):
        """Standard dispatcher to run worker-thread callbacks on the main GUI thread."""
        if callback:
            try:
                callback(code, status)
            except Exception as e:
                self._log_in_ui(f"⚠ Callback Error: {e}\n")
    ...
    def build_ytdl_tab(self):
        ...
        v = QVBoxLayout(tab)
        v.setSpacing(4)
        self.tabs.addTab(scroll, "📺 YT-DLP")
        ...
        btn_row = QHBoxLayout()
        self.btn_yt_dl = QPushButton("⬇ Download")
        self.btn_yt_dl.clicked.connect(self.ytdl_start)
        btn_row.addWidget(self.btn_yt_dl)
        
        self.btn_yt_update = QPushButton("🔄 Update yt-dlp")
    ...
    def ytdl_start(self):
        ...
        # Add Thumbnails/Metadata/Subs from UI (Wait, need to add UI elements first in build_ytdl_tab)
        if hasattr(self, 'yt_thumb') and self.yt_thumb.isChecked(): args.append("--write-thumbnail")
        if hasattr(self, 'yt_meta') and self.yt_meta.isChecked(): args.append("--write-info-json")
        if hasattr(self, 'yt_subs') and self.yt_subs.isChecked(): args.append("--write-subs")
            
        self.yt_runner = YTDLRunner(args)
        self.yt_runner.log_message.connect(self.append_log) # Connect Logging
    ...
    def build_bitrate_tab(self):
        ...
        v = QVBoxLayout(tab)
        v.setSpacing(6)
        self.tabs.addTab(scroll, "🧮 Bitrate Calc")
        ...
        btn_apply = QPushButton("✅ Apply to Convert Tab")
    ...
    def apply_bitrate(self):
        if hasattr(self, 'last_calc_bitrate'):
            ...
            self.change_category(0) # Video Basics
    ...
    def build_sync_tab(self):
        ...
        v = QVBoxLayout(tab)
        v.setSpacing(6)
        self.tabs.addTab(scroll, "⏳ A/V Sync")
        ...
        btn_row = QHBoxLayout()
        self.sync_preview_btn = QPushButton("👁 Preview")
        ...
        self.sync_queue_btn = QPushButton("🕒 Queue")
        ...
        self.sync_run_btn = QPushButton("▶ Fix Sync")
    ...
    def sync_run(self):
        ...
        cmd = self.preview.toPlainText().strip()
        if cmd:
            self.runner.execute(cmd, on_finished=lambda ec, st: self._on_finished("Sync Fix", ec, st))
    ...
    def setup_player_placeholder(self, layout):
        ...
        self.player_video = QLabel("🎞️ Multimedia Backend Missing\n(Internal Player Unavailable)")
    ...
    def play_with_ffplay(self, filepath):
        ...
        try:
             ...
             self._log_in_ui(f"🎬 Opening with FFplay: {Path(filepath).name}\n")
        except Exception as e:
